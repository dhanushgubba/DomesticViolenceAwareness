...
<body>
    ...
    <table id="usersTable" align="center" border="2" class="table table-striped">
        ...
        <c:forEach items="${shelters}" var="shelter">
        	<tr>
        		<td><c:out value="${shelter.id}"></c:out></td>
        		<td><c:out value="${shelter.name}"></c:out></td>
        		<td><c:out value="${shelter.type}"></c:out></td>
        		<td><c:out value="${shelter.support}"></c:out></td>
        		<td><c:out value="${shelter.contact}"></c:out></td>
        		<td><c:out value="${shelter.location}"></c:out></td>
        		<td><c:out value="${shelter.description}"></c:out></td>
        		<td>
        			<img src="displayshelterimage?id=${shelter.id}" alt="Shelter Image" width="100" height="100" />		
        		</td>
        		<td class="action-links-container">
    				<a href="<c:url value='delete?id=${shelter.id}' />" class="action-link delete-link">Delete</a>
    				<a href="<c:url value='update?id=${shelter.id}' />" class="action-link update-link">Update</a>
				</td>
